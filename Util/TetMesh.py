...
class TetMesh:
    ...
    def __init__(self,
                 ...
                 scale_list=[],
                 density=1.0,
                 is_static=False,tetidx_perm = False):
        ...
        for i in range(len(model_name_list)):
            ...
            np.random.seed(None)
            tet_indices_np_temp = np.array(mesh.cells[0].data, dtype=int) + self.num_verts

            if (tetidx_perm) :
                tet_indices_original = tet_indices_np_temp.copy()
                tet_indices_np_temp = np.array([np.random.permutation(row) for row in tet_indices_np_temp])
                is_different = not np.array_equal(tet_indices_np_temp, tet_indices_original)
                print(tet_indices_original[:5])
                print(tet_indices_np_temp[:5])
                print("Permutation changed anything?", is_different)

            offset = self.num_verts * np.ones(shape=4, dtype=int)
            ...
            self.num_tetras += mesh.cells[0].data.shape[0]
            offsets.append(self.num_verts)
        ...
        node = ti.root.dense(ti.i, self.num_tetras)
        node.place(self.invDm, self.V0)
        ti.root.dense(ti.ij, (self.num_tetras, 4)).place(self.tet_indices)
        ...
        face_indices_np = f[indxs[count == 1]]
        face_indices_np.astype(int)
        num_faces = face_indices_np.shape[0]

        self.face_indices = ti.field(dtype=int)
        ti.root.dense(ti.i, 3 * num_faces).place(self.face_indices)
        self.face_indices.from_numpy(face_indices_np.reshape(3 * num_faces))
        self.init()
